...
int main() {
	vector<double> vectorIn, hKernel, vectorOut;
	double* dIn, * dOut;
	int inCols, inRows;
	int kDim, kRadius;
	int outCols, outRows;
	...
	*/
	...
	vectorIn.assign(image.data, image.data + image.total());
	inCols = image.cols;
	inRows = image.rows;
	vectorOut.resize(inCols * inRows, 0);
	...
	*/
	...
	errCatch(cudaMalloc((void**)&dIn, vBytes(vectorIn)));
	errCatch(cudaMemcpy(dIn, vectorIn.data(), vBytes(vectorIn), cudaMemcpyHostToDevice));
	errCatch(cudaMalloc((void**)&dOut, vBytes(vectorOut)));
	errCatch(cudaMemcpy(dOut, vectorOut.data(), vBytes(vectorOut), cudaMemcpyHostToDevice));
	...
	*/
	...
	errCatch(cudaMemcpy(vectorOut.data(), dOut, vBytes(vectorOut), cudaMemcpyDeviceToHost));
	errCatch(cudaDeviceSynchronize());
	...
	*/
	...
	vector<int> toInt(vectorOut.begin(), vectorOut.end()); // Converting from double to integer matrix
