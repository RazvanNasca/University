...
struct Cmp{
    ...
    bool operator()(int x, int y)       /// asta ii sintaxa :( scuze ... seamana cu ce am facut la Istvan
    {
        return D[x] > D[y];             /// ordoneaza nodurile din coada dupa costul nodului
    }

};

priority_queue < int, vector <int>, Cmp> Q; /// coada cu prioritate in care elementele se vor adauga in functie de Cmp
...
void Dijkstra(int nod)
{
    ...
    while(!Q.empty())
    {
        int nodCurent = Q.top();    /// extragem nodul din varful cozii
        Q.pop();                    /// il eliminam
