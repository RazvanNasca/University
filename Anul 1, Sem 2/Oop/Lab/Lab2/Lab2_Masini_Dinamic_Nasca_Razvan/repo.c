...
{	
	if (cars->masina[i].marca && strlen(m.marca))
		cars->masina[i].marca = (char*)realloc(cars->masina[i].marca, strlen(m.marca) + 1);
	if(cars->masina[i].categorie && strlen(m.categorie))
		cars->masina[i].categorie = (char*)realloc(cars->masina[i].categorie, strlen(m.categorie) + 1);
	

	if (cars->masina[i].marca && cars->masina[i].categorie)
	{
...
int Actiune(char* actiune, int  poz, MyList* cars)
{
...
void sortare(char* criteriu, char* ordine, MyList* cars, FunctieComparareMarca cmpM, FunctieComparareCategorie cmpC)
{
	/*
